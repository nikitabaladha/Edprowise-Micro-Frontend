...
const ExcelSheetModal = ({
  ...
  const handleImportClick = () => {
    ...
    if (feesTypes.length === 0) {
      toast.error(
        "No one-time fees types available. Please configure fees types."
      );
      return;
    }
    ...
    for (const [index, payload] of validatedData.entries()) {
      try {
        ...
        if (!response.hasError) {
          toast.success(
            `One-time fees for class imported successfully (Row ${index + 1}).`
          );
        } else {
          toast.error(
            response.message ||
              `Failed to import one-time fees for row ${index + 1}.`
          );
        }
      } catch (err) {
        const errorMessage =
          err?.response?.data?.message ||
          `Error importing one-time fees for row ${index + 1}.`;
