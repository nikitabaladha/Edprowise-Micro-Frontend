    ...
    for (const [index, payload] of validatedData.entries()) {
      try {
        ...
        if (!response.hasError) {
          toast.success(
            `Board registration fees for class imported successfully (Row ${
              index + 1
            }).`
          );
        } else {
          toast.error(
            response.message ||
              `Failed to import board registration fees for row ${index + 1}.`
          );
        }
      } catch (err) {
        const errorMessage =
          err?.response?.data?.message ||
          `Error importing board registration fees for row ${index + 1}.`;
