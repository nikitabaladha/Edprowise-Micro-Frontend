...
const ExcelSheetModal = ({
  ...
  const handleImportClick = () => {
    ...
    if (feesTypes.length === 0) {
      toast.error(
        "No school fees types available. Please configure fees types."
      );
      return;
    }
          ...
          jsonData.forEach((row, index) => {
            ...
            const sections = row.Sections
              ? String(row.Sections)
                  ...
                  .map((s) => s.trim())
              : [];
            const installmentName = row.InstallmentName
              ? String(row.InstallmentName).trim()
              : "";
            ...
            if (isNaN(parsedDueDate.getTime())) {
              toast.error(
                `Row ${
                  index + 1
                }: Invalid due date "${dueDate}". Use YYYY-MM-DD format.`
              );
              hasError = true;
              return;
            }
            ...
            if (!feesType) {
              toast.error(
                `Row ${index + 1}: Invalid fees type "${feesTypeName}".`
              );
              hasError = true;
              return;
            }
            ...
            let installment = groupedData[key].installments.find(
              (inst) =>
                inst.name.toLowerCase() === installmentName.toLowerCase()
            );
            if (!installment) {
              installment = {
    ...
    for (const [index, payload] of validatedData.entries()) {
      try {
        ...
        if (!response.hasError) {
          toast.success(
            `Fee structure for class imported successfully (Row ${index + 1}).`
          );
        } else {
          toast.error(
            response.message ||
              `Failed to import fee structure for row ${index + 1}.`
          );
        }
      } catch (err) {
        const errorMessage =
          err?.response?.data?.message ||
          `Error importing fee structure for row ${index + 1}.`;
  ...
  const handleDownloadDemo = () => {
    if (classes.length === 0 || feesTypes.length === 0) {
      toast.error(
        "No classes or fees types available to include in demo sheet."
      );
      return;
    }
    ...
    const guidelines = [
      ...
      [
        `• Available Fees Types: ${feesTypes
          .map((f) => f.feesTypeName)
          .join(", ")}.`,
      ],
    ];
