...
const BoardFeeRegistration = () => {
    ...
    const fetchFees = async () => {
      ...
      try {
        const feesResponse = await getAPI(
          `/get-board-registration-fees-byIds/${schoolId}/${academicYear}/${selectedClass}/${
            selectedSection || ""
          }`,
          {},
          true
        ...
        if (Array.isArray(feesData)) {
          ...
          toast.info(
            "No board registration fees found for the selected class/section."
          );
          setBoardFees({});
        }
      } catch (error) {
  ...
  const handlePaidClick = async (event, student) => {
    ...
    try {
      ...
      const receiptData = {
        ...student,
        receiptNumberBrf:
          response?.data?.data?.[0]?.receiptNumberBrf ||
      ...
      navigate(
        "/school-dashboard/fees-module/fees-receipts/board-registration-fees/receipts",
        {
          state: {
            student: receiptData,
    ...
    try {
      ...
      const receiptStudents = selectedStudents
        ...
        .map((student, index) => ({
          ...student,
          receiptNumberBrf:
            response?.data?.data?.[index]?.receiptNumberBrf ||
            `REC-${student._id}-${Date.now()}`,
          admissionFees: student.feesAmt,
          concessionAmount: 0,
          finalAmount: student.feesAmt,
  ...
  const handleViewReceipt = (student) => {
    const receiptData = {
      ...student,
      receiptNumber:
        student.receiptNumber || `REC-${student._id}-${Date.now()}`,
      admissionFees: student.feesAmt,
      concessionAmount: 0,
      finalAmount: student.feesAmt,
    ...
    <div className="container-fluid">
      <div className="container">
        <h1 className="h3 fw-bold text-dark mb-4">
          Board Registration Fee Collection
        </h1>
