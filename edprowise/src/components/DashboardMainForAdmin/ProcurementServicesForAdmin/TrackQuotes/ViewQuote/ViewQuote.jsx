...
const ViewQuote = () => {
  ...
  const handleVenderStatusUpdate = async (newStatus) => {
    try {
      ...
      const response = await putAPI(
        `${process.env.REACT_APP_PROCUREMENT_QUOTE_PROPOSAL_SERVICE}/update-vender-status?enquiryNumber=${encodedEnquiryNumber}&sellerId=${sellerId}`,
  ...
  const generateQuotePDF = async (enquiryNumber, sellerId) => {
    ...
    try {
      ...
      const response = await getAPI(
        `${process.env.REACT_APP_PROCUREMENT_QUOTE_PROPOSAL_SERVICE}/generate-quote-pdf?schoolId=${schoolId}&sellerId=${sellerId}&enquiryNumber=${encodedEnquiryNumber}`,
        { responseType: "blob" },
        true
      );
